...
- **The operator running the swarm** should get glanceable confidence, not a reason to dig through logs: a scheduled heartbeat quietly reviews real commit diffs, a single pinned health-report issue updates in place instead of spamming, and the [Health Dashboard](#health-dashboard-dashboard) gives the same picture live in a browser - worst-status-first, so a real problem is never buried under quiet ones. **Update (2026-09-18):** a hub URL is now actually live for the first time - the Apps Script path (see "Alternative: Deploy Without Vercel" below), confirmed with a real `doPost()` round trip and a real canary run that wrote genuine entries to this hub's own `ai_decision_log.json`/`usage/default.json` (commits `b9d7d2c`/`6ff175e`). `DOCS_VS_CODEBASE.md` #7 is now **BUILT**, not PARTIAL. What's still not live: no spoke's `call-hub.yml` points at this URL yet, and the two Workspace Flows that actually answer queued rows haven't been built by hand yet - so today's operator experience is "the pipe is proven end-to-end," not yet "a scheduled heartbeat is quietly running against a real spoke." The Vercel path described in this bullet remains undeployed.
...
**Update (2026-09-18):** the Apps Script backend is now genuinely deployed and live (see "Alternative: Deploy Without Vercel" below) - the paragraph below describing "never run for real end-to-end" no longer applies to that path, kept here as the historical reason this skill was built. This skill still has its own, different value even with a live backend: it never needs `AI_API_KEY`/`AI_BASE_URL` (never set, on either path) or a human-built Workspace Flow, and it can review a spoke on demand, in-session, without waiting on that spoke's own schedule or a Flow to answer a queued row.

Historical context (accurate as of writing, before the update above): the Autonomous Agent above had never run for real end-to-end - it needed a deployed backend (Vercel or Apps Script, neither ever actually live) *and* a configured `AI_API_KEY`/`AI_BASE_URL` (never set). This skill sidesteps both without touching either deployment path: instead of the handler calling out to an external AI API, the Claude session invoking the skill *is* the AI step, directly, reusing every other rail above (decision-log dedup, dry-run default, rate cap, decision logging) exactly as designed - same schema, same fetch order, same all-or-nothing local-context quirk (see below), just with the "call an external `/chat/completions` endpoint" step replaced by real reasoning performed in-session.
...
- **Caller authentication, required for every registered tenant.** A request body must include `callerKey`, checked against the resolved tenant's `callerKeyRef`. `"default"` - the tenant every spoke currently maps to - has one configured (`tenants.json`'s `callerKeyRef: "env:DEFAULT_TENANT_CALLER_KEY"`); a request with no key or the wrong one gets a 401. This closed a real gap: the hub's `gas/` deployment is a publicly-reachable ("Anyone") web app, and before this, a caller who knew the URL could trigger a review for any registered owner/repo using `GLOBAL_GITHUB_TOKEN` with zero credential of their own. `setup_spoke.py`'s generated `call-hub.yml` forwards a `TENANT_CALLER_KEY` secret as `callerKey` - set it to the current `DEFAULT_TENANT_CALLER_KEY` value before triggering a spoke's workflow for real, or every run gets rejected.
- **Unregistered owner/repo is rejected outright, before any GitHub call runs.** `resolveTenantIdForSpoke()` no longer falls back to `"default"` for an owner/repo that isn't in `spokes.json` - it returns no tenant at all, and the request gets a 403. `owner`/`repo` are caller-supplied and spoofable, so this doesn't replace the caller-key check above; it bounds what a leaked/guessed key can be used against to this hub's own registered spokes, never an arbitrary repo on GitHub that `GLOBAL_GITHUB_TOKEN` can read.
...
2. In GitHub Repository Settings → Secrets and variables → Actions:
   ...
   - Add `TENANT_CALLER_KEY`: **required**. Every spoke currently maps to the `"default"` tenant, which has a `callerKeyRef` configured (see [Multi-Tenancy](#multi-tenancy-tenantsjson)) - get the current value from whoever operates the hub. A request with no key or the wrong one gets a 401.
...
4. In the Apps Script IDE (`clasp open`) → Project Settings → Script Properties, set `ADMIN_SETTINGS_TOKEN` (a long random string, not a memorable password), `GLOBAL_GITHUB_TOKEN`, `DRY_RUN_MODE`, and `RATE_CAP_PER_REPO_PER_DAY`. `QUEUE_SHEET_ID` is optional here - leave it unset and the first real request or scheduled harvest run creates the queue spreadsheet automatically and persists its ID for you (see "Native Workspace Inference" below); set it yourself only if you want the queue to live on a specific existing Sheet instead. `AI_API_KEY`/`AI_MODEL`/`AI_BASE_URL` are **not** needed for this deployment path - see that same section for why.
...
**Native Workspace Inference (no AI API key, no GCP project):** unlike `api/autonomous_agent.js`/`api/recursive_learning.js` (which call a `chat/completions`-shaped AI endpoint directly), `gas/`'s copies never call an AI API at all. Instead they hand off to a **human-built Google Workspace Studio Flow** using its own native "Ask Gemini" step - the same pattern already proven in this account's KOS repo (cas-ccps's Flow 2 redesign) for a Workspace org where GCP is disabled account-wide, so a developer-facing Gemini API key can't even be issued. The hand-off, one row at a time, via `gas/review_queue.js`:

1. `processRequest()`/`runRecursiveLearning()` do all their normal context-gathering and skip logic exactly as before (dedup, quota, no-diff-skip), then write one flat row with `ReadyStatus = "READY"` into a `ReviewQueue`/`LearningQueue` tab, instead of calling an AI endpoint. The HTTP response is `"Queued"`, not a real outcome - `call-hub.yml`'s own run just shows that, not "found 2 issues."
2. **A human builds the Flow, once, in the Workspace UI** - nothing in this repo can do this step, Studio/Flow has no deploy API:
   - The queue spreadsheet itself doesn't need to be created by hand: the first real request (`processRequest()`/`runRecursiveLearning()`) or the first scheduled `harvestReviewResults()`/`harvestLearningResults()` run creates it automatically (`SpreadsheetApp.create()`, locked against a concurrent double-create - see `gas/review_queue.js`'s `ensureQueueSpreadsheetCreated_`), persists its ID into `QUEUE_SHEET_ID` for you, and creates the `ReviewQueue`/`LearningQueue` tabs on it with headers. Find the link on the settings page (`?endpoint=settings&token=...`, next to the `QUEUE_SHEET_ID` field) once it exists, or in that run's Apps Script execution log. Setting `QUEUE_SHEET_ID` yourself beforehand (step 4 above) skips auto-creation and points the pipeline at that Sheet instead - useful if you want the queue to live somewhere specific (a shared Drive, a particular folder) rather than wherever the executing account's My Drive puts a freshly-created file.
   - In Google Workspace's Flow builder (Drive → New → More → Google Flow, or the Flow app depending on your Workspace edition), create a Flow triggered by **"Spreadsheet row added or updated"** on that Sheet's `ReviewQueue` tab, filtered to `ReadyStatus = READY`.
   - Add an **Extract** step reading that row's `PromptText` column.
   - Add an **Ask Gemini** step (Flow's own native model step - no API key field, no GCP project picker) using the extracted `PromptText` as its instruction.
   - Add an **Update spreadsheet row** step writing Gemini's response into that same row's `GeminiFullOutput` column and setting `ReadyStatus = EVALUATED`.
   - Repeat for a second Flow watching the `LearningQueue` tab, same shape.
3. Once ADMIN-set up, run `installReviewQueueTriggers()` once from the Apps Script IDE's function picker (or `clasp run installReviewQueueTriggers`) - installs the two time-based triggers (`runHarvestReviewResults` every 5 minutes, `runHarvestLearningResults` every 30) that pick up `EVALUATED` rows and do everything the old inline AI-response handling did: validate the shape, apply `DRY_RUN_MODE`/the rate cap, write the decision log, and (live mode) file the issue or open the PR.
4. Also run `installDeployVersionReportTrigger()` once, the same way - see "Catching deploy drift" below for what it's for.

**Known, disclosed limitation:** a green "Run Completed" in Studio/Flow's own log proves nothing by itself - confirmed the hard way building cas-ccps's Flow 2, where a lookup step matching zero rows still logged success. Call `checkReviewQueueLiveness()`/`checkLearningQueueLiveness()` (Apps Script IDE function picker, or the settings page) to distinguish "no Flow has ever answered" from "the Flow is just slow" - a bare green checkmark in Workspace's own UI cannot tell you which. They also catch a specific, easy-to-make Flow-building mistake: writing the answer into `GeminiFullOutput` but forgetting to also set `ReadyStatus` to the literal `EVALUATED` in that same "update spreadsheet row" step - harvest only ever scans for `EVALUATED`, so a row like that has a real answer sitting right there and is never picked up. Reported as `stuckAtReady`, not silently counted as healthy - the exact bug KOS's own equivalent check (`checkFlow2Binding()`, cas-ccps) found and fixed first.

**Testing the pipeline before, or without, a live Flow:** ported from the rest of KOS's own "Flow Doctrine" (`meta/FLOW_DOCTRINE.md`) - liveness (above) is one of six roles that doctrine says every Flow-based system needs; these three cover the rest. All are Apps Script IDE function-picker-runnable (`clasp run <name>` also works), and log a JSON result to the execution log:

- **Canary** (`runReviewQueueCanaryNow()` / `runLearningQueueCanaryNow()`) - proves the queue → harvest → finalize wiring actually works end to end, without needing a live Flow at all. Enqueues one synthetic row, simulates the one step only a human-built Flow normally performs (writing `GeminiFullOutput` + `ReadyStatus = EVALUATED`), then runs the real harvest/finalize code against it and reports whether it landed `HARVESTED`. Safe to run against an already-live deployment: it always answers itself with the safest possible response (`has_findings: false` / `has_proposal: false`), so it can never reach the issue-creation or PR-opening code paths regardless of this deployment's real `DRY_RUN_MODE` - and it uses a synthetic `commitSha` (`CANARY-<timestamp>-<random>`)/`tenantId` (`canary`), never a real one, so it can't collide with or poison the real decision-log dedup key a genuine review of that commit would use.
- **Binding check** (`runReviewQueueBindingCheckNow()` / `runLearningQueueBindingCheckNow()`) - compares the sheet's actual header row against `RQ_HEADERS`/`LQ_HEADERS`, the one canonical column map `gas/review_queue.js` declares. Catches a human accidentally reordering, renaming, or deleting a header cell while wiring a Flow's Extract/Update-row step against the sheet by hand - a Flow step bound to the wrong column silently reads or writes the wrong data, no error, no red Studio run.
- **Fixture installer** (`installReviewQueueFixtureNow()` / `installLearningQueueFixtureNow()`) - appends one clearly-synthetic, idempotency-checked row (`Owner`/`Repo` = `_fixture`/`_fixture`, or `TenantId` = `_fixture`) with a real, well-formed `PromptText` explaining its purpose, so a human building a Flow has something stable to point a first test run at without waiting for real `call-hub.yml` traffic or a monthly learning pass. The review fixture is safe to answer any way, even live: `_fixture`/`_fixture` isn't a real GitHub repo, so even a `has_findings: true` answer fails cleanly at issue-creation (caught by harvest's own per-row error handling, marks `ERROR_HARVEST_FAILED`, nothing else happens). The learning fixture is **not** the same - `finalizeTenantLearning_` always opens its PR against this hub repo itself, so a live `has_proposal: true` answer creates a real (reviewable, not auto-merged, but real) PR here; the row's own `PromptText` says to prefer `{"has_proposal": false}` unless you're deliberately testing that path.

**Catching deploy drift (git says one thing, the live deployment says another):** ported from KOS's own `tools/deploy-drift/` - the same class of failure as `checkReviewQueueLiveness()`, but at the deployment layer instead of the Flow layer: `git push` landing is not the same thing as a live `clasp push` + `clasp deploy -i <id> -V <n>` actually happening, and no Claude session working on this repo can run either of those two commands itself (no browser-based `clasp login` from a container) - every future `gas/` change needs a human to remember to push+deploy it, completely out of band from git. `gas/deploy_version_report.js` self-reports gas/'s live version once a day (`installDeployVersionReportTrigger()`, same one-time IDE-run setup as `installReviewQueueTriggers()`) via a `repository_dispatch` event, reusing `GLOBAL_GITHUB_TOKEN` already in Script Properties rather than a second, separately-scoped token - it already has full write access to this hub repo, so letting it also fire one more event type doesn't expand what a leak of it could do. `.github/workflows/deploy-drift.yml` reacts, and `scripts/deploy-drift.js` compares the reported SHA against what `scripts/deploy-drift-expected-marker.js` says git expects (the most recent commit touching any of gas/'s tracked files), opening/updating/closing a single pinned `Deploy drift: gas/ Apps Script backend` issue - same update-in-place pattern `scripts/watchdog.js` already uses. The marker itself (`gas/deploy_version_marker.js`) has to be stamped as its own commit, separate from the real code change, since a commit can't embed its own SHA:
```
1. Commit your real gas/ code change(s) normally.
2. node scripts/deploy-drift-stamp.js
3. Commit ONLY the resulting gas/deploy_version_marker.js change, by itself.
4. clasp push, then clasp deploy -i <id> -V <n> to actually promote it.
```
Skipping step 2/3, or combining them with step 1, breaks the match - by design, exactly the kind of slip this mechanism exists to surface, not silently tolerate. **Update (2026-09-18):** `installDeployVersionReportTrigger()` has now been run successfully against a real deployment (confirmed in that project's Executions log) - the daily trigger is genuinely installed. **Still unverified:** `reportDeployVersion()` itself hasn't fired yet (no `deploy-drift.yml` run exists on GitHub yet - `0` runs as of this writing), so the actual self-report round trip to GitHub is still unconfirmed. Run `clasp run runReportDeployVersionNow` (or trigger it from the Apps Script IDE) to test it now rather than waiting up to 24h for the daily schedule, then check `.github/workflows/deploy-drift.yml`'s run history.

**Rotating config later without reopening the Apps Script IDE:** once step 4's `ADMIN_SETTINGS_TOKEN` is set, visit `<your Web App URL>?endpoint=settings&token=<that value>` for a small settings page (`gas/settings.js`) that reads/writes `GLOBAL_GITHUB_TOKEN`, `DRY_RUN_MODE`, `RATE_CAP_PER_REPO_PER_DAY`, `QUEUE_SHEET_ID`, `HUB_GITHUB_OWNER`/`HUB_GITHUB_REPO`, `DEFAULT_TENANT_CALLER_KEY` (the shared caller key every registered spoke's `TENANT_CALLER_KEY` secret has to match - see [Multi-Tenancy](#multi-tenancy-tenantsjson)), plus `AI_API_KEY`/`AI_MODEL`/`AI_BASE_URL` (kept settable only for a possible future direct-call fallback mode - nothing on the deployed pipeline reads them today). This doesn't remove step 4 - `ADMIN_SETTINGS_TOKEN` itself still has to be bootstrapped via Script Properties first, and it's the one value this page will never show or let you change, so a leaked page token can't mint its own replacement. Secret fields (`AI_API_KEY`, `GLOBAL_GITHUB_TOKEN`, `DEFAULT_TENANT_CALLER_KEY`) render masked and start blank - leaving one blank and saving keeps its current value, it never gets cleared. A wrong or missing token gets a generic response either way, so probing the URL doesn't confirm the page even exists.

**Live diagnostics, not just a form:** the page also validates `GLOBAL_GITHUB_TOKEN` against the real API the moment it loads (a real, cheap call - `GET /rate_limit`) and shows a green "✓ valid" or red "✗ <reason>" next to it. This exists because a bad `GLOBAL_GITHUB_TOKEN` sat undetected through 5 straight failed `health-report.yml` runs before anyone noticed - opening this page now catches that the moment you open it, instead of after N silent failures. The `AI_API_KEY` check still exists on this page too (harmless if left blank on the native-inference path above - it's an unused, legacy field now).

**Verified locally, and now live for the first time (2026-09-18):** `scripts/dev-test-gas-*.mjs` cover the ported decision logic, the GitHub REST mapping, `Code.js`'s request routing, `review_queue.js`'s enqueue/harvest/liveness behavior, and the settings page's token gate/masking/allowlist behavior against hand-rolled fakes - the same testing discipline as everything else in this repo, and they already caught one real defect (`autonomous_agent.js`/`recursive_learning.js` both declaring the same constant, a silent `SyntaxError` the moment both files shared one real Apps Script project's scope) before any real deployment existed.

What's now actually happened, against a real `clasp push` + deployed Apps Script project: a real `doPost()` round trip (`{"owner":"adamberneche-afk","repo":"Mothership","mode":"debug"}` → `{"status":"Queued",...,"dryRun":true}`), real auto-creation of the `QUEUE_SHEET_ID` spreadsheet, and a real canary run (`runReviewQueueCanaryNow`) that exercised the full enqueue → harvest → finalize path end to end, including genuine writes to this hub's own `ai_decision_log.json` and `usage/default.json` (commits `b9d7d2c`/`6ff175e`) - proof the credential, decision-log, and usage-log plumbing all work for real, not just against fakes.

What still hasn't happened: an actual Workspace Flow built by hand and wired to a real Sheet (nothing can automate this step - Studio/Flow has no deploy API), and no real spoke's `call-hub.yml` points at this deployment yet. Build the Flow, confirm `dryRun: true` responses, and confirm a Flow-answered row actually harvests correctly before pointing any spoke's schedule at it.

**Continuous deployment (`.github/workflows/deploy-apps-script.yml`):** once the one-time deployments in step 5 above exist, `clasp push`/`clasp deploy` no longer has to happen by hand for every change. A push to `main` updates the production deployment (`GAS_PROD_DEPLOYMENT_ID`) in place; a pull request targeting `main` updates a separate, persistent staging deployment (`GAS_STAGING_DEPLOYMENT_ID`) instead - Apps Script's equivalent of a PR preview, except the URL stays stable across runs rather than minting a new one each time (`clasp deploy -i <id>`, not a bare `clasp deploy`). Both deployments are created once by hand via the IDE (step 5); this workflow only ever updates them, never creates a new one. Needs four repository secrets: `CLASPRC_JSON` (clasp's own OAuth credential file, base64-encoded - clasp itself writes it after `clasp login`; this workflow decodes it to both the older `~/.clasprc.json` and newer `~/.config/clasp/.clasprc.json` locations, since which one a given clasp version actually reads hasn't been confirmed from this session), `GAS_SCRIPT_ID`, and one deployment id + Web App URL pair per environment (`GAS_PROD_DEPLOYMENT_ID`/`GAS_WEB_APP_URL`, `GAS_STAGING_DEPLOYMENT_ID`/`GAS_STAGING_WEB_APP_URL`). Until `CLASPRC_JSON`/`GAS_SCRIPT_ID` are actually set, every real step is gated behind a check for them and the workflow reports a clean skip on every push/PR instead of a permanent red failure - so a fresh clone of this repo doesn't start life with a perpetually-failing required check. After every deploy, `scripts/smoke-test.js` hits `?endpoint=health` (this file's `renderHealthResponse()`, a deliberately trivial `{status: 'ok'}` with zero PropertiesService/UrlFetchApp/GitHub/AI calls, so the check can't itself be the thing that's broken) with a few retries for propagation lag before the job is considered failed - the same generic script a future non-Apps-Script CD pipeline could reuse unchanged. A `workflow_dispatch` run is routed to staging or production by which branch it's actually run against (`github.ref`), not by treating "not a pull_request event" as "must be production" - dispatching this by hand against a non-`main` branch lands on staging, same as a real PR would. The whole workflow shares one concurrency group, not scoped per-branch: `clasp push` always pushes to the one shared Apps Script project (there's no per-branch namespace the way a Vercel preview gets its own isolated deployment), so a PR's staging deploy and main's production deploy running at the same time would otherwise race on that shared push - one global group serializes every deploy against this project instead, newest cancelling oldest. Known, disclosed limitation: a `pull_request` from a fork doesn't receive repository secrets, so a staging deploy only works for PRs from branches within this same repository.

**Not removed:** `api/*.js` and the Vercel path stay in this repo untouched, and still call an AI endpoint directly (unaffected by any of the above - the native-inference queue is `gas/`-only). Dropping Vercel entirely - deleting `api/`, `setup_hub.py`'s Vercel-flavored generation, the Vercel-specific docs above - is a deliberate follow-up once the Apps Script path has actually been verified live, not bundled into adding it.
...
- `scripts/dev-test-handler.mjs`, `dev-test-recursive-learning.mjs`, `dev-test-prune-logs.mjs`, `dev-test-health-report.mjs`, `dev-test-watchdog.mjs` - one per capability, run with `node scripts/dev-test-*.mjs` or all at once via `npm test`.
...
**Beyond `ci.yml`:** `.github/dependabot.yml` watches this repo's own npm dependency and every workflow file's action versions weekly. `.github/workflows/codeql.yml` runs CodeQL (javascript-typescript + python + actions) on every push/PR to `main` plus a weekly schedule. `scripts/watchdog.js` + `.github/workflows/watchdog.yml` run actionlint against every workflow file and check that every `on.schedule` workflow's most recent run actually succeeded, publishing findings to one pinned issue updated in place - the same "is the swarm's own machinery running" question the Hub Health panel above answers live, but as a standing GitHub issue instead of something someone has to remember to open the dashboard to see.
