...
The hub can serve more than one customer/organization ("tenant") without pooling their data or their GitHub credentials. This started as architecture-and-data-model work; a later pass added a real GitHub App credential path and a genuinely self-service onboarding flow on top of it (see `lessons.md`'s dated entries for the full history). Still disclosed as not fully built: no generic encrypted secrets store (`kv:` remains an unbuilt placeholder, deliberately re-scoped - see below), no customer-facing usage dashboard, and `setup_hub.py`/`dashboard/`'s core scaffold still assumes one hub, one operator. Real multi-tier pricing and a real Stripe Customer Portal integration are now built (see below) - what's still missing is Mothership's own code reacting to a subscription-lifecycle webhook event (`customer.subscription.deleted`/`.updated`, failed-payment dunning): the Portal itself can handle cancellation and Stripe's own dunning emails handle failed payments once the operator enables those in the Stripe Dashboard, but nothing here listens for or acts on either.

- **Per-tenant GitHub credential, not one shared master token.** `spokes.json` now carries a `tenantId` per entry (every spoke registered before this feature existed was migrated to `tenantId: "default"`, which still resolves to `GLOBAL_GITHUB_TOKEN` - zero behavior change for the pre-existing single-tenant setup). `tenants.json` (new, hub root) holds one entry per tenant: `{ tenantId, name, status, plan, quota: { reviewsPerMonth }, githubCredentialRef, callerKeyRef?, stripeCustomerId?, installationId?, suspendedReason?, createdAt }`. `lib/secrets.js` is the single canonical module every Node-side caller resolves `githubCredentialRef`/`callerKeyRef`/`plans.json`/`tenants.json`/`spokes.json` through (dedupe from what used to be 6 byte-identical copies scattered across `api/`/`scripts/`) - a `scheme:value` pointer format:
...
A standalone, installable PWA - additive, not part of the request-handling path (a PWA can't receive GitHub Actions' POST mid-cron-job the way the two endpoints above do). Since Mothership and every spoke are public repos, `dashboard/app.js` reads `spokes.json` and each spoke's `ai_decision_log.json`/issue list straight from `api.github.com` client-side and renders the same metrics `health-report.js`'s `buildReportForSpoke` already computes - skip rate, per-outcome breakdown, inferred live/dry-run status - live in a browser instead of waiting for the weekly pinned issue. It also cross-references `tenants.json`: a suspended tenant's spoke (see the Multi-Tenancy section's suspension flow below) is forced to the most severe status/color and sorted first, regardless of what its own decision log happens to show - a suspension is ground truth and must never read as "quiet" just because nothing else has happened on that spoke recently.
...
**Deliberately `workflow_dispatch`-only, no schedule** (`.github/workflows/doctor.yml`) - this project's own investigation into its health started because scheduled workflows were failing silently with nobody watching; adding another scheduled job here would risk the identical failure mode this tool exists to catch. Run it manually when setting up a new spoke, rotating a credential, or troubleshooting.
...
| `HUB_GITHUB_OWNER`, `HUB_GITHUB_REPO` | Defaults to `adamberneche-afk`/`Mothership` | Optional - only needed if you've forked this repo under a different owner/name; every hub-registry read/write (`tenants.json`, `spokes.json`, `plans.json`) uses these to know where the hub itself lives. |
| `ONBOARDING_FAILURE_URL`, `ONBOARDING_PENDING_APPROVAL_URL` | Defaults to `/onboarding-failed.html`/`/onboarding-pending-approval.html` | Optional - override where `api/github_app_callback.js` redirects on a rejected or pending-approval install; the defaults already match the static pages this repo ships, so most setups never need to set these. |
