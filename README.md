...
- **The operator running the swarm** should get glanceable confidence, not a reason to dig through logs: a scheduled heartbeat quietly reviews real commit diffs, a single pinned health-report issue updates in place instead of spamming, and the [Health Dashboard](#health-dashboard-dashboard) gives the same picture live in a browser - worst-status-first, so a real problem is never buried under quiet ones.
