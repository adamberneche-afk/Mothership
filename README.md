...
```
Skipping step 2/3, or combining them with step 1, breaks the match - by design, exactly the kind of slip this mechanism exists to surface, not silently tolerate. **Not yet wired up live**, same disclosed status as the rest of `gas/`: verified locally (`scripts/dev-test-deploy-drift.mjs`, `scripts/dev-test-gas-deploy-version-report.mjs`) against a scratch git repo and a fake octokit, not yet run against a real `clasp push`+deploy.
...
**Verified locally, not yet live:** `scripts/dev-test-gas-*.mjs` cover the ported decision logic, the GitHub REST mapping, `Code.js`'s request routing, `review_queue.js`'s enqueue/harvest/liveness behavior, and the settings page's token gate/masking/allowlist behavior against hand-rolled fakes - the same testing discipline as everything else in this repo, and they already caught one real defect (`autonomous_agent.js`/`recursive_learning.js` both declaring the same constant, a silent `SyntaxError` the moment both files shared one real Apps Script project's scope) before any real deployment existed. What hasn't happened yet is an actual `clasp push` + live dispatch against a real Apps Script project, or an actual Workspace Flow built by hand and wired to a real Sheet - do both and confirm `dryRun: true` responses before pointing any spoke's schedule at it.
