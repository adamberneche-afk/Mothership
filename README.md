...
4. In the Apps Script IDE (`clasp open`) → Project Settings → Script Properties, set `ADMIN_SETTINGS_TOKEN` (a long random string, not a memorable password), `GLOBAL_GITHUB_TOKEN`, `DRY_RUN_MODE`, `RATE_CAP_PER_REPO_PER_DAY`, and `QUEUE_SHEET_ID` (see "Native Workspace Inference" below for where that ID comes from). `AI_API_KEY`/`AI_MODEL`/`AI_BASE_URL` are **not** needed for this deployment path - see that same section for why.
...
2. **A human builds the Flow, once, in the Workspace UI** - nothing in this repo can do this step, Studio/Flow has no deploy API:
   - Create a new, blank Google Sheet anywhere in your Drive. Copy its ID (the long string in its URL between `/d/` and `/edit`) into `QUEUE_SHEET_ID` above - the first real request or `harvestReviewResults()` run creates the `ReviewQueue`/`LearningQueue` tabs on it automatically, with headers.
