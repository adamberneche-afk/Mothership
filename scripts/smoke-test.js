...
// actually alive and responding before deploy-apps-script.yml considers
// the deploy successful. Hits the dedicated ?endpoint=health liveness
// route - see gas/Code.js's renderHealthResponse for what it's checking -
// never the AI-calling endpoints, which need real credentials to do
// anything meaningful and could have side effects.
//
...
// after a deploy completes (an Apps Script deployment version taking a
// moment to become the active one) is normal, expected behavior, not a
// real problem - treating a single transient blip as a hard CI failure
// would create exactly the kind of noisy false-failure this project has
// been careful to avoid elsewhere (see doctor.js's deliberately narrow,
// honestly-scoped checks).
//
// Written generically (not Apps-Script-specific) on purpose: the same
// {status: 'ok'} contract and optional expectedCommit comparison would
// work unchanged against any other deployment target this hub grows a CD
// pipeline for later.
//
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (!url) {
    ...
    // Generic escape hatch for a target that needs one extra header to
    // reach its own health route (e.g. an auth/bypass header) - both must
    // be non-empty, or no header is sent at all.
