...
// health check deploy-apps-script.yml runs before considering a deploy
// successful.
//
...
async function testHealthyResponseIsOk() {
  ...
  const result = await checkHealth('https://example.com/health', { fetchImpl });
...
async function testTimeoutIsUnhealthyNotThrown() {
  ...
  const result = await checkHealth('https://example.com/health', { fetchImpl, timeoutMs: 20 });
  check('not ok', result.ok === false);
...
async function testNetworkErrorIsUnhealthyNotThrown() {
  ...
  let result;
  try {
    result = await checkHealth('https://example.com/health', { fetchImpl });
  } catch (e) {
    threw = true;
  }
...
async function testCommitMismatchIsUnhealthy() {
  ...
  const result = await checkHealth('https://example.com/health', { fetchImpl, expectedCommit: 'new-sha' });
  check('not ok', result.ok === false);
...
async function testHeadersAreForwardedWhenSupplied() {
  console.log('an optional headers object (e.g. an auth/bypass header) is forwarded to fetchImpl');
  const fetchImpl = makeFakeFetch(() => jsonResponse(200, { status: 'ok' }));
  await checkHealth('https://example.com/health', { fetchImpl, headers: { 'x-bypass-token': 'secret-token' } });
  check('fetch received the header', fetchImpl.calls[0].options.headers?.['x-bypass-token'] === 'secret-token');
}
...
async function testNoHeadersOptionMeansNoHeadersSent() {
  ...
  const fetchImpl = makeFakeFetch(() => jsonResponse(200, { status: 'ok' }));
  await checkHealth('https://example.com/health', { fetchImpl });
