...
| 13 | Make `setup_hub.py`'s embedded file copies regenerable | Mothership | Open — `ci.yml` diffs 14 tracked files against copies embedded verbatim in `setup_hub.py`. Dependabot cannot see those copies, so **every** dependency PR touching one fails the drift check and can never merge unaided. Four were fixed by hand on 2026-09-20; the next one will fail the same way. Needs a sync script with real round-trip tests — the JS entries have sharp escaping hazards (a `\n` inside a JS string literal means something else inside a non-raw Python triple-quote), and a half-right version would corrupt the installer silently. |
